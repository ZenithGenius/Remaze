...
//   private boolean showScoreboard = false; Flag to control scoreboard
   ...
   public static int coinage = 25;// was static
   public static int health = 100;// was static
   public static int level = 1;// was static
   ...
   public int spawnFrame;
    ...
    public Screen(Frame var1) {
        this.frame = var1;
      this.setLayout(null);
      this.mobNumber = level * 10; //at first 3
      this.mobSpawned = 0;
      this.spawnTime = 2000;
      this.spawnFrame = 0;
   ...
   private void showPauseMenu() {
    ...
    //closeCurrentGameFrame(); // Close the active frame
  ...
  private void resetGameState() {
    ...
    coinage = 25;    // Reset coinage to initial value
    health = 100;    // Reset health to initial value
    level = 1;       // Reset level to initial value
    killCount = 0;   // Reset kill count to initial value

    // Reinitialize static arrays or collections that hold game state
    mobs = new Mob[100];   // Reset the mobs array
    for(int i = 0; i < mobs.length; i++) {
        mobs[i] = new Mob();  // Reinitialize each mob
    }

    // Reinitialize any other game components that hold state
    room = new Room();
    save = new Save();
    store = new Store();
}
  ...
  public void closeCurrentGameFrame() {
    SwingUtilities.invokeLater(() -> {
        if (this.frame != null) {
            this.frame.dispose(); // Dispose of the specific game frame
            this.frame = null;    // Remove the reference to allow garbage collection
        }
    });
    }
          ...
          if (!isPaused) { // Check if the game is not paused
              if (!isFirst && health > 0) {
                  ...
                  if (killCount == this.mobNumber && health > 0){
                    ...
                    // new ScoreBoard(coinage, health, killCount, level);
                    
                    // Run the ScoreBoard creation on the EDT
                    SwingUtilities.invokeLater(() -> {
                        closeCurrentGameFrame(); 
                        new ScoreBoard(coinage, health, killCount, level);
                    });
            ...
            for(int var1 = 0; var1 < mobs.length; ++var1) {
                if (!mobs[var1].inGame) {
                    ...
                    if (this.mobSpawned == this.mobNumber - 1) { // Ensure we leave space for the boss
                        ...
                        mobs[var1].spawnMob(Value.mobBoss, 2000 * level, 22 * level, 100, 14 / level); // 2000 atleast
                       //System.out.println("Boss Spawned! Health: " + mobs[this.mobNumber].health + " Tower Damage: " + mobs[this.mobNumber].towerDamage + " Count: " + this.mobSpawned + " At Speed " + mobs[this.mobNumber].walkSpeed);
                        ...
                        if (this.mobSpawned % 2 == 0) {
                            ...
                            mobs[var1].spawnMob(Value.mobWarrior, 1200 * level, 22 * level, 10, 16 / level); // 1200 Second type of normal mob (mobWarrior)
                            //System.out.println("Normal Spawned! Health: " + mobs[var1].health + " Tower Damage: " + mobs[var1].towerDamage + " Count: " + this.mobSpawned + " At Speed " + mobs[var1].walkSpeed);
                        }
