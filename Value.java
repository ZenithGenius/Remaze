public class Value {
    public static int groundGrass = 0;
    public static int groundRoad = 1;
    public static int airAir = -1;
    public static int airHome = 0;
    public static int airTrash = 1;
    public static int airTowerLaser = 2;
    public static int airTowerFrost = 3;
    public static int mobAir = -1;
    public static int mobAngry = 0;
    public static int mobWarrior = 2;
    public static int mobBoss = 1; //first 1
    public static int[] deathReward = new int[]{3, 5, 10}; //frst 2,5
 
    public Value() {
    }
 }
 